...
{
    public partial class Form房屋物件 : Form
    {
        public Form房屋物件()
        {
            InitializeComponent();
        ...
        private void btn加入預約列表_Click(object sender, EventArgs e)
        {
            if (houseDataGridView.SelectedRows.Count > 0)
            {
                foreach (DataGridViewRow row in houseDataGridView.SelectedRows)
                {
                    DataRowView dataRowView = (DataRowView)row.DataBoundItem;
                    DataRow newRow = Globalvar.Housetable.NewRow();
                    newRow["id"] = dataRowView["id"];
                    newRow["物件名稱"] = dataRowView["物件名稱"];
                    newRow["屋齡"] = dataRowView["屋齡"];
                    newRow["坪數"] = dataRowView["坪數"];
                    newRow["行政區域"] = dataRowView["行政區域"];
                    newRow["物件型態"] = dataRowView["物件型態"];
                    newRow["車位"] = dataRowView["車位"];
                    newRow["房數"] = dataRowView["房數"];
                    newRow["價格"] = dataRowView["價格"];
                    newRow["社區名稱"] = dataRowView["社區名稱"];

                    // 添加其他你需要的列
                    Globalvar.Housetable.Rows.Add(newRow);
                    MessageBox.Show("已幫您加入預約列表中");
                }
            }
            else
            {
                MessageBox.Show("請選擇想預約看屋的房子加入列表");
            }

        }

        private void btn查看預約列表_Click(object sender, EventArgs e)
        {
            

        }

        private void btn查看預約列表_Click_1(object sender, EventArgs e)
        {
